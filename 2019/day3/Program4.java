...
class Program4 {
    ...
    public static void main(final String[] args) {
        ...
        for (String intersection : intersections) {
            ...
            //int distance = Math.abs(Integer.parseInt(resultSplit[0])) + Math.abs(Integer.parseInt(resultSplit[1]));

            System.out.println(intersection);
            //System.out.println("distance: " + distance);

            Set<String> walkToIntersection = walkToIntersection(firstStepList, intersection);

            for (String step : walkToIntersection) {
                //String[] resultSplit = step.split("\\.");
                //int distance = Math.abs(Integer.parseInt(resultSplit[0])) + Math.abs(Integer.parseInt(resultSplit[1]));
    ...
    public static Set<String> walkToIntersection(Set<String> stepList, String intersection) {
        Set<String> stepsToIntersectionList = new HashSet<String>();

        //reset postions for new wire
        posX = 0;
        posY = 0;

        for (String instruction : stepList) {
            doSteps(stepList, instruction, null);
        }
        return stepsToIntersectionList;
    }
    ...
    public static Set<String> doSteps(Set<String> stepList, String instruction, String intersection) {
        ...
        for (int j = 0; j < steps; j++) {
            ...
            // stop when intersection is reached
            if (intersection != null && step.equals(intersection)) {
                return stepList;
            }
        }
        return stepList;
